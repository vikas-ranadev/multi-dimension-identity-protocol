...
describe('client.preparePayment', () => {
  ...
  it('throws an error when the balance is insufficient', async () => {
    ...
    const opts = {
      ...
      utxoData: {
        unspents: [{
          txid: 'mock-txid', outputIndex: 0, satoshis: 10000000, rawTx: { hex: 'mock-raw-tx' },
        }],
        fee: 0.0001,
        nulldataFee: 0.0001,
      },
    ...
    }).toThrowError('Error: Insufficient balance. required amount: 1.0001, available amount: 0.10000000');
  });
  ...
  it('throws an error when rawdata is too long', async () => {
    ...
    }).toThrowError('Error: payment information can have at most 144 characters');
  });
