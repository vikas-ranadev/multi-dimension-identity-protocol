...
const handleSingleTxDetail = async (txDetailParam, address) => {
  let totalInputValue = 0;
  let totalOutputValue = 0;
  const txDetail = txDetailParam;
  txDetail.transactionType = omniTxTypes.incoming;
  const inputTxDetailPromises = txDetailParam.vin.map(async (vin) => {
...
api.getTransactionHistory = async (req, res) => {
  ...
  try {
    ...
    const promises = txDetails.map(async (txDetailParam) => {
      return handleSingleTxDetail(txDetailParam, address)
    });
