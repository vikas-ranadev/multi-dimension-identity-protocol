...
const getTransactionDetails = async (txids, getConfirmations = false) => {
  ...
  const promises = txids.map(async (txid) => {
    try {
      const txDetail = (await btcClient('getrawtransaction', [txid, true]))
        .result;
...
api.getTransactionHistory = async (req, res) => {
  ...
  try {
    ...
    const promises = txDetails.map(async (txDetailParam) => {
      let totalInputValue = 0;
      let totalOutputValue = 0;
      const txDetail = txDetailParam;
      txDetail.transactionType = omniTxTypes.incoming;
      const tempPromises = txDetailParam.vin.map(async (vin) => {
