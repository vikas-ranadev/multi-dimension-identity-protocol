...
const debug = require('debug')('mdip:api');
...
api.prepareTransaction = async (req, res) => {
  ...
  try {
    ...
    debug('api.prepareTransaction', errorT);
...
api.issueNewClaim = async (req, res) => {
  try {
    ...
    debug('api.issueNewClaim', errorT);
    return res.status(500).send({ result: false, error: errorT.message });
  }
};
...
api.updateDID = async (req, res) => {
  try {
    ...
    try {
      ...
      debug('api.updateDID gettxout', errTxo);
...
api.deleteDID = async (req, res) => {
  try {
    ...
    try {
      ...
      debug('api.deleteDID gettxout', errTxo);
      return res.status(400).send({
        error: true,
        message: 'DID already revoked.',
      });
...
api.getutxos = async (req, res) => {
  try {
    ...
    debug('[getaddress utxos]', unspents.length);
    ...
    if (totalAmount >= Number(sanitizedFee)) {
      ...
      debug('[getaddress utxos 2]', unspents.length);
      ...
      debug('[check getRawTxPromiseArr]', getRawTxPromiseArr);
      const promisesResp = await Promise.allSettled(getRawTxPromiseArr);
      debug('[check promisesResp]', promisesResp);
  ...
  try {
    ...
    for (let i = 0; i < txids.length; i++) {
      try {
        ...
        debug('api.checkconfs getrawtransaction', errGetTx);
    ...
    debug('api.fundETHAddress', errorT);
    return res.status(500).send({ result: false, error: errorT.message });
  }
};
...
api.getTransactionHistory = async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < txs.length; i++) {
      const txid = txs[i];
      // eslint-disable-next-line no-await-in-loop
      const txDetail = (await btcClient('getrawtransaction', [txid, true], isOmni)).result;
      let totalInputValue = 0;
      let totalOutputValue = 0;
      txDetail.transactionType = 'incoming';
      for (let j = 0; j < txDetail.vin.length; j++) {
        const vin = txDetail.vin[j];
        // eslint-disable-next-line no-await-in-loop
        const inputTxDetail = (await btcClient('getrawtransaction', [vin.txid, true], isOmni)).result;
        const { value, scriptPubKey } = inputTxDetail.vout[vin.vout];
        totalInputValue += value;
        if (address === scriptPubKey.addresses[0]) {
          txDetail.transactionType = 'outgoing';
        }
      }
    ...
    debug('api.getTransactionHistory', errT);
    if (errT && errT.message) {
      return res.status(500).send({ error: errT.message, result: null });
    }
    return res.status(500).send({ error: 'Internal error.', result: null });
...
api.testMempoolAccept = async (req, res) => {
  ...
  try {
    ...
    debug('api.testMempoolAccept', JSON.stringify(result));
    error = result[0]['reject-reason'];
  } catch (errT) {
    debug('api.testMempoolAccept', errT);
    if (errT && errT.message) {
      error = errT.message;
    } else {
