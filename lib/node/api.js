...
const getTransactionDetails = async (txids, getConfirmations = false) => {
  ...
  const invalidTxids = [];
  const confirmations = [];
  const txPromises = txids.map((txid) => btcClient('getrawtransaction', [txid, true]));
...
const handleSingleTxDetail = async (txDetailParam, address, isOmni) => {
  ...
  const inputTxDetailPromises = txDetailParam.vin.map((vin) => btcClient('getrawtransaction', [vin.txid, true], isOmni));
