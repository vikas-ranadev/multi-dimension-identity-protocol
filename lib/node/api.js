...
async function parseBlocks(txid, vout, blockhash, isOmni) {
  ...
  const { tx, nTx, nextblockhash } = (
    await util.btcClient('getblock', [blockhash, 2])
  ).result;
  for (let x = 0; x < nTx; x++) {
...
async function getTxsChain(txid, vout, isOmni) {
  try {
    await util.btcClient('gettxout', [txid, vout], isOmni);
    return {};
  } catch (errTxo) {
    const getRawTx = (await util.btcClient('getrawtransaction', [txid, true], isOmni))
      .result;
...
api.getServerInfo = async (req, res) => {
  ...
  try {
    const serverInfo = await util.checkConnections();
    return res.status(200).send({ error: null, result: serverInfo });
  } catch (error) {
...
api.prepareTransaction = async (req, res) => {
  ...
  try {
    ...
    let fee = 0;
    const unspents = (
      await util.btcClient('listunspent', [1, 9999999, [didCreator]])
    ).result;
    if (unspents.length) {
      const inputs = [];
      const outputs = {};
      fee = util.helpers.calculateFee(unspents.length, NO_OF_TX_OUTPUTS);
      let totalAmount = 0;
      unspents.forEach((x) => {
      ...
      if (totalAmount >= Number(amount) + Number(fee)) {
        ...
        const rawTx = await util.btcClient('createrawtransaction', [
          inputs,
          outputs,
        ]);
        if (rawTx.result) {
    ...
    fee = util.helpers.calculateFee(FINAL_NO_OF_TX_INPUTS, FINAL_NO_OF_TX_OUTPUTS);
    return res.status(500).send({
      result: null,
      error: i18n(
...
api.broadcast = async (req, res) => {
  try {
    ...
    if (blockchain === 'eth') {
      ...
      sentTx = await util.btcClient('sendrawtransaction', [signedTx], isOmni);
    }
    return res.status(200).send({
      error: null,
...
api.updateDID = async (req, res) => {
  try {
    ...
    const { txid, utxoIndex } = await txrefToTxid(finalTxRef, util.btcClient);
    let txoCall;
    try {
      txoCall = await util.btcClient('gettxout', [txid, utxoIndex], isOmni);
    } catch (errTxo) {
    ...
    if (txo) {
      ...
      const fee = util.helpers.calculateFee(1, 1);
      let totalAmount = txo.value;
      if (totalAmount < fee) {
        const unspents = (
          await util.btcClient('listunspent', [
            1,
            9999999,
      ...
      const rawTx = await util.btcClient('createrawtransaction', [inputs, outputs], isOmni);
      const signedTx = await util.btcClient('signrawtransactionwithkey', [
        rawTx.result,
        [privateKey],
      ], isOmni);
      if (signedTx.result && signedTx.result.complete) {
        const sentTx = await util.btcClient('sendrawtransaction', [
          signedTx.result.hex,
        ], isOmni);
        return res.status(400).send({
          error: null,
...
api.deleteDID = async (req, res) => {
  try {
    ...
    const txRef = did.split(':')[2].split(splitByStr)[1];
    const finalTxRef = network === 'mainnet' ? `tx1:${txRef}` : `txtest1:${txRef}`;
    const { txid, utxoIndex } = await txrefToTxid(finalTxRef, util.btcClient, isOmni);
    let txoCall;
    try {
      txoCall = await util.btcClient('gettxout', [txid, utxoIndex], isOmni);
    } catch (errTxo) {
...
api.readDID = async (req, res) => {
  try {
    ...
    const { vout } = (
      await util.btcClient('getrawtransaction', [txid, true], isOmni)
    ).result;
    let OP_RETURN = null;
...
api.getutxos = async (req, res) => {
  try {
    ...
    const isOmni = blockchain === OMNI_BLOCKCHAIN;
    let { result: unspents } = await util.btcClient('getaddressutxos', [{ addresses: [address] }], isOmni);
    const totalAmount = unspents.reduce((prev, curr) => prev + curr.satoshis, 0);
    const { fee, nulldataFee } = await util.helpers.calculateFee(
    ...
    const mempoolTx = (await util.btcClient('getaddressmempool', [{ addresses: [address] }], isOmni)).result;
    ...
    if (totalAmount >= Number(sanitizedFee)) {
      ...
      for (let i = 0; i < unspents.length; i++) {
        ...
        getRawTxPromiseArr.push(util.btcClient('getrawtransaction', [txid], isOmni));
      }
...
const getTransactionDetails = async (txids, getConfirmations = false) => {
  ...
  const invalidTxids = [];
  const confirmations = [];
  const txPromises = txids.map((txid) => util.btcClient('getrawtransaction', [txid, true]));
...
api.fundETHAddress = async (req, res) => {
  ...
  try {
    ...
    const { gas, gasPrice } = await util.helpers.calculateFeeETH(fromAddr, toAddr, ethBalance);
    ...
    if (ethBalanceDecimal < Number(amount) + Number(fee)) {
      return res.status(400).send({
        error: `Current ETH balance ${ethBalanceDecimal} is less than required amount + fee ${Number(amount) + Number(fee)}`,
        result: null,
      });
    }
...
api.getTxDetails = async (req, res) => {
  ...
  try {
    const isOmni = blockchain === OMNI_BLOCKCHAIN;
    const getRawTx = (await util.btcClient('getrawtransaction', [txid, true], isOmni)).result;
    const getBlock = (await util.btcClient('getblock', [getRawTx.blockhash, 1], isOmni)).result;
  ...
  try {
    ...
    if (blockchain === BTC_BLOCKCHAIN || isOmni) {
      ...
      const { success, total_amount: totalAmount } = (
        await util.btcClient('scantxoutset', ['start', [{ desc: `addr(${address})` }]], isOmni)
      ).result;
...
const handleSingleTxDetail = async (txDetailParam, address, isOmni) => {
  ...
  const inputTxDetailPromises = txDetailParam.vin.map((vin) => util.btcClient('getrawtransaction', [vin.txid, true], isOmni));
  const inputTxDetailArray = await Promise.all(inputTxDetailPromises);

  for (let i = 0; i < txDetail.vin.length; i++) {
    const inputTxDetail = inputTxDetailArray[i].result;
    const { value, scriptPubKey } = inputTxDetail.vout[txDetail.vin[i].vout];
    totalInputValue += value;
    if (scriptPubKey.addresses && address === scriptPubKey.addresses[0]) {
...
api.getTransactionHistory = async (req, res) => {
  ...
  try {
    const isOmni = true;
    const txids = (await util.btcClient('getaddresstxids', [address])).result;
    const mempoolTx = (await util.btcClient('getaddressmempool', [{ addresses: [address] }], isOmni)).result;
    const txs = txids.concat(mempoolTx.map((x) => x.txid));
    const txDetails = await getTransactionDetails(txs);
    const promises = txDetails.map(
      (txDetailParam) => handleSingleTxDetail(txDetailParam, address, isOmni),
    );
    const allTxDetails = await Promise.all(promises);
    const { result: unspents } = await util.btcClient('getaddressutxos', [{ addresses: [address] }], isOmni);
...
api.testMempoolAccept = async (req, res) => {
  ...
  let error = null;
  try {
    const { result } = (await util.btcClient('testmempoolaccept', [[signedTx]]));
    if (result[0].allowed) {
      return res.status(200).send({
...
api.getTxidIPNS = async (req, res) => {
  ...
  try {
    const getRawTx = (await util.btcClient('getrawtransaction', [txid, true])).result;
    const data = util.helpers.decodeNullDataIPNS(getRawTx.vout);
