...
api.broadcast = async (req, res) => {
  try {
    ...
    if (blockchain === 'eth') {
      ...
      sentTx = await btcClient('sendrawtransaction', [signedTx]);
    }
    return res.status(200).send({
      error: null,
...
api.getutxos = async (req, res) => {
  try {
    ...
    const { fee, nulldataFee } = await helpers.calculateFee(unspents.length, 3);
...
api.getTxDetails = async (req, res) => {
  ...
  try {
    ...
    const getBlock = (await btcClient('getblock', [getRawTx.blockhash, 1])).result;
