...
async function parseBlocks(txid, vout, blockhash) {
  ...
  ).result; // 2(highest) signifies verbosity level of this command.
  ...
  try {
    if (blockchain === 'eth') {
      ...
      if (type === 'create') {
        ...
        console.log('check raw transaction', rawTx);
  ...
  try {
    ...
    if (success && totalAmount >= Number(sanitizedFee)) {
      ...
      return res.status(200).send({
        ...
        result: { unspents, fee: sanitizedFee },
      });
    }
    return res.status(200).send({
      error: true,
...
api.fundETHAddress = async (req, res) => {
  ...
  console.log('checkzz', toAddr, fromAddr, privateKey, amount, chainID);
  if (!fromAddr || !privateKey || !amount || !fromAddr || !chainID) {
