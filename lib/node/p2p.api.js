...
p2p.addSubs = async (req, res) => {
  try {
    ...
    debug('[p2p][addSubs]', error, JSON.stringify(result));
    if (error) {
      if (error.code === -32603) {
...
p2p.broadcast = async (req, res) => {
  try {
    ...
    debug('[resp] [broadcast]', JSON.stringify(resp));
    return res.status(200).send({
...
p2p.fetchBroadcasts = async (req, res) => {
  try {
    ...
    debug(`[resp] [address] ${broadcasterAddress} [fetchBroadcasts] ${resp?.result?.inboxMessages?.length}`);
