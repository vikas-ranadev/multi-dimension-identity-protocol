 ...
 */
client.prepareTransaction = async (mdipUrl, didInputs) => {
  try {
    const { blockchain } = didInputs;
    if (blockchain === 'btc') {
      /** Not Needed as createAndSignTx is used by BTC. */
      return null;
    }
    if (blockchain === 'eth') {
      ...
      const rawTx = await client.call({
        ...
        method: 'POST',
        params: {
          blockchain, type, didDoc, publicKey: didInputs.didCreator,
        },
      });
      return rawTx;
    }
 ...
 * @returns {object}
 */
client.signTx = async (privKey, rawTx, pubKey) => {
  try {
