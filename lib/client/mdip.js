 ...
 * @returns {Array}
 */
client.getAddressTxid = async (mdipUrl, blockchain, address) => {
  try {
    if (blockchain === 'omni') {
      const TxnObject = await client.call({
        url: `${mdipUrl}/getaddresstxids`,
        method: 'GET',
        params: { address },
      });
      return TxnObject.result;
    }
    return null;
  } catch (error) {
    throw new Error(error);
