 ...
 * @param {string} fromAddr sender/change address
  ...
  try {
    ...
    for (let i = 0; i < unspents.length; i++) {
      ...
      psbt.addInput({
        hash: txid,
        index: vout,
        sequence: 0xfffffffd, // make the txn RBF
        nonWitnessUtxo: Buffer.from(hex, 'hex'),
      });
...
client.decodeSignedTx = (signedTx, opts) => {
  ...
  try {
    ...
    tx.ins = tx.ins.map((input) => ({ ...input, txid: input.hash.reverse().toString('hex') }));

    tx.outs = tx.outs.map((output) => {
      try {
        const address = bitcoin.address.fromOutputScript(output.script, nw);
        return { ...output, address };
      } catch (e) {
        // ignore nulldata outs
        return output;
      }
    });
