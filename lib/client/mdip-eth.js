...
client.createDIDTx = async (didDoc, publicKey, privateKey, didInputs, mdipUrl) => {
  try {
    ...
    if (blockchain === 'eth') {
      let signedTx = await ethDID.create(privateKey, didDoc, publicKey);
...
client.updateDIDdoc = async (did, newdidDoc, publicKey, privateKey, didInputs) => {
  try {
    ...
    if (blockchain === 'eth') {
      ...
      const signedTx = await ethDID.setMetadata(privateKey, publicKey, cid, newdidDoc);
      signedTx = signedTx.data;
      const sentTx = await client.call({
        url: `${mdipUrl}/${'broadcast'}`,
        method: 'POST',
...
client.updateDIDowner = async (did, publicKey, privateKey, newOwner, didInputs) => {
  try {
    ...
    if (blockchain === 'eth') {
      ...
      const cid = part3.substr(4, part3.length);
      const signedTx = await ethDID.setController(privateKey, publicKey, newOwner, cid);
      signedTx = signedTx.data;
      const sentTx = await client.call({
        url: `${mdipUrl}/${'broadcast'}`,
        method: 'POST',
...
client.deleteDID = async (did, publicKey, privateKey, didInputs) => {
  try {
    ...
    if (blockchain === 'eth') {
      ...
      const cid = part3.substr(4, part3.length);
      const signedTx = await ethDID.deleteDID(privateKey, publicKey, cid);
      signedTx = signedTx.data;
      const sentTx = await client.call({
        url: `${mdipUrl}/${'broadcast'}`,
        method: 'POST',
