...
didUtils.prepareDidInputs = async (blockchain, masterRoot, childIndexes, network, mdipURL) => {
  ...
  const utxos = await getUtxos(mdipURL, blockchain, network, publicAddr1);
  return {
    keyPairs: {
      [child1]: {
...
didUtils.createAndSignTx = (
  { unspents, fee, didCreator: creator, didUpdater: updater, blockchain, network },
  privKey,
) => {
  ...
  const change = parseInt((totalAmount - (sendAmt + fee)) * 10 ** 8, 10);
  if (change > 0) {
    psbt.addOutput({
      address: creator,
      value: change,
