...
const calculateFee = async (inputs, outputs, isOmni) => {
  try {
    ...
    if (feerate) {
      const txSize =
        inputs * WEIGHTED_INPUT + outputs * WEIGHTED_OUTPUT + inputs;
...
const checkConnections = async () => {
  ...
  const eth = props.path().eth;
  const omni = props.path().omni;
  const { testnet } = props.path().mdip;
  const p2pTransport = props.path()['p2p_transport'];
