...
const debug = require('debug')('mdip:utils');
...
const bmUrl = props.get('p2p_transport.url');
const bmBUrl = props.get('p2p_transport.broadcast');
const bmUser = props.get('p2p_transport.username');
const bmPass = props.get('p2p_transport.password');
...
const linkToIPNS = async (ipfsAddr, keyName) => {
  try {
    ...
    const resp = await coreClient.name.publish(ipfsAddr, {
      key: keyName,
      resolve: true,
      lifetime: '2400h',
    });
...
const createIPNSRecord = async (
  ...
  try {
    ...
    debug('check entryData', entryData);
    return entryData;
  } catch (error) {
    debug('check error', error);
    return null;
 ...
 * @returns {object}
 */
const bmClient = async (method, params = [], broadcast = false) => {
  const URL = broadcast && bmBUrl ? bmBUrl : bmUrl;
  const username = bmUser;
  const password = bmPass;
  debug('bmClient URL', URL);
  debug('bmClient broadcasting', !! broadcast && bmBUrl);
  if (!URL || !username || !password) {
...
const xmlToJSON = (xmlData) => xmlParser.xml2json(xmlData, { compact: true });

const listAddressesXMLRequest = () => `<methodCall>
<methodName>listAddresses</methodName>
<params></params>
</methodCall>
`;

const addSubscriptionXMLRequest = ({ address }) => `<methodCall>
...
const sendMessageXMLRequest = ({
  ...
  <params>
      ...
      <subject>
          <value>${Buffer.from(subject).toString('base64')}</value>
      </subject>
      <message>
          <value>${Buffer.from(message).toString('base64')}</value>
      </message>
  </params>
</methodCall>`;

const sendBroadcastXMLRequest = ({
  fromAddress,
  subject,
  message,
  encodingType,
}) => `<methodCall>
  <methodName>sendBroadcast</methodName>
  <params>
      <fromAddress>
          <value>${fromAddress}</value>
      </fromAddress>
      ...
      <encodingType>
          <value>${Buffer.from(encodingType).toString('base64')}</value>
      </encodingType>
  </params>
</methodCall>`;

const getAllInboxMessagesXMLRequest = () => `<methodCall>
...
const checkConnections = async () => {
  ...
  if (omni) {
    ...
    serverInfo.btc = { btcNodeInfo, omniLayerInfo };
  }
  if (!p2pTransport) {
    throw new Error('p2p transport not provided');
  }
  const bmResp = await bmClient('clientStatus', []);
  const {
    result: { networkStatus },
  } = bmResp;
  if (networkStatus !== 'connectedAndReceivingIncomingConnections') {
    throw new Error(
      'Bitmessage node cannot connect to the network',
      networkStatus,
    );
  }
  serverInfo.p2p_transport = { bm: bmResp.result };
  debug(serverInfo);
  return serverInfo;
};
...
Util.xmlToJSON = xmlToJSON;
Util.sendMessageXMLRequest = sendMessageXMLRequest;
Util.listAddressesXMLRequest = listAddressesXMLRequest;
Util.sendBroadcastXMLRequest = sendBroadcastXMLRequest;
Util.addSubscriptionXMLRequest = addSubscriptionXMLRequest;
Util.getAllInboxMessagesXMLRequest = getAllInboxMessagesXMLRequest;
