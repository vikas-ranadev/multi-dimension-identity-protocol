 ...
 * @returns {object}
 */
const bmClient = (xmlRequestBody, givenBMUrl, givenBMUser, givenBMPass) => {
  const URL = givenBMUrl || bmUrl;
  const username = givenBMUser || bmUser;
  const password = givenBMPass || bmPass;
  ...
  return new Promise((resolve, reject) => {
    request(req, (err, _, result) => {
      ...
      const parsedOutput = xmlParser.xml2json(result, { compact: true, trim: true });
      const parsedJSON = JSON.parse(parsedOutput);
      console.log('[bmClient parsedJSON]', parsedJSON.methodResponse.params.param);
      const proResponse = parsedJSON.methodResponse.params.param.value.string._text;
      console.log('[bmClient proResponse]', proResponse);
      if (isJSON(proResponse)) {
        resolve(JSON.parse(proResponse));
      } else {
        resolve(proResponse);
      }
    });
...
const sendMessageXMLRequest = ({ toAddress, fromAddress, subject, message }) => `<methodCall>
...
const sendBroadcastXMLRequest = ({ fromAddress, subject, message, encodingType }) => `<methodCall>
