/** TODO: Switch to commonJS modules and remove babel. */
...
const mnemonic =
  'aware acquire nature aware true moon thumb transfer bleak lend win march';
...
function util() {
  ...
  const addrIndex = 0;
  const endUserPubKey = getLeafAddressesFromDerivedPubKey(
    derivedPubKey,
    addrIndex
  );
  const endUserPrivKey = getLeafAddressPrivKey(
    rootNode,
    'm/44/0/0/0',
    addrIndex
  );
...
async function createDIDRefactored(rootNodeObj) {
  ...
  try {
    ...
    const derivedPubKey = getDerivedRootsPubKey(
      rootNodeObj,
    ...
    if (sentTxn && sentTxn.success) {
      ...
      let tmpTxnHash =
        'f3d8db389e242a52889afb1a286cf73434118cab161dcf5545854ca7535960cf';
      /**Remove txtest */
      const didTxRef = bech32.encode('txtest', tmpTxnHash);
      console.log('[DID Part 3]', didTxRef, '\n');
      return {
        did: `did:${didMethod}:${didTxRef.split('txtest')[1]}`,
        pubKey: p0,
        privKey: s0,
        btcAddr: b0,
      };
...
async function sendTxn({ fromAddr, toAddr, amount, fee, privKey, OP_RETURN }) {
  ...
  try {
    ...
    let unspents = (await btcClient('listunspent', [1, 9999999, [fromAddr]]))
      .result;
    if (unspents.length) {
      let inputs = [],
        outputs = {},
      ...
      if (totalAmount >= amount + Number(givenFee)) {
        ...
        let signedTx = await btcClient('signrawtransactionwithkey', [
          rawTx.result,
          [decPrivKey],
        ]);
        console.log('[Check signed Tx]', signedTx);
        if (signedTx.result && signedTx.result.complete) {
          let sentTx = await btcClient('sendrawtransaction', [
            signedTx.result.hex,
          ]);
