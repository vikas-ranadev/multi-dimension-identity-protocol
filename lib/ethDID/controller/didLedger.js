...
async function getDID(req, res) {
    try {
        var callPromise = await contract.methods.dids(web3.utils.toHex(req.body.did)).call().then(async function (result) {
            return result
        });
...
async function getController(req, res) {
    try {
        var callPromise = await contract.methods.getController(web3.utils.toHex(req.body.did)).call().then(async function (result) {
            return result
        });
        res.send({ success: true, data: callPromise })
    } catch (err) {
...
async function setController(req, res) {
    try {
        ...
        let tx
        var callPromise = await contract.methods.setController(web3.utils.toHex(req.body.did), req.body.newAddress).send(overrideOptions).then(function (transaction, err) {
            if (!err) tx = transaction
            else tx = err
        });
        res.send({ success: true, data: tx })
 ...
 async function setMetadata(req, res) {
    try {
        ...
        let bytes32 = utils.getBytes32FromMultiash(req.body.cid);
        let tx
        var callPromise = await contract.methods.setMetadata(web3.utils.toHex(req.body.did),bytes32.digest).send(overrideOptions).then(function (transaction, err) {
            if (!err) tx = transaction
            else tx = err
        });
        res.send({ success: true, data: tx })
...
async function deleteDID(req, res) {
    try {
        ...
        let tx
        var callPromise = await contract.methods.deleteDID(web3.utils.toHex(req.body.did)).send(overrideOptions).then(function (transaction, err) {
            if (!err) tx = transaction
            else tx = err
        });
        res.send({ success: true, data: tx })
